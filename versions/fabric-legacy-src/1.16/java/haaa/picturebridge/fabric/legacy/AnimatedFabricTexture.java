...
final class AnimatedFabricTexture implements AutoCloseable {
    ...
    AnimatedFabricTexture(MinecraftClient client, DecodedImage decoded) {
        ...
        this.texture = new NativeImageBackedTexture(new NativeImage(decoded.width(), decoded.height(), true));
        upload(decoded.frames().get(0).image());
        this.identifier = client.getTextureManager().registerDynamicTexture("picturebridge", texture);
        this.nextFrameAtNanos = System.nanoTime() + decoded.frames().get(0).durationMillis() * 1_000_000L;
    }
    ...
    private void upload(BufferedImage frame) {
        ...
        for (int y = 0; y < frame.getHeight(); y++) for (int x = 0; x < frame.getWidth(); x++)
            pixels.setPixelColor(x, y, argbToAbgr(frame.getRGB(x, y)));
        texture.upload();
    }

    private static int argbToAbgr(int color) { return color & 0xFF00FF00 | color >> 16 & 0xFF | (color & 0xFF) << 16; }
