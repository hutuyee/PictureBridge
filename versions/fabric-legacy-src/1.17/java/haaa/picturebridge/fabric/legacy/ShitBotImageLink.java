...
public final class ShitBotImageLink {
    ...
    public static URI find(Style style) {
        ...
        Text text = hover == null ? null : hover.getValue(HoverEvent.Action.SHOW_TEXT);
        if (text == null) return null;
        String marker = text.getString().toLowerCase(Locale.ROOT);
        return marker.contains("qq 图片") || marker.contains("qq image") || marker.contains("qq 表情")
                || marker.contains("qq expression") || marker.contains("qq emoji") ? uri : null;
    }
}
