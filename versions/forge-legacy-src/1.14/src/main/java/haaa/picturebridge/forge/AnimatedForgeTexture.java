...
final class AnimatedForgeTexture implements AutoCloseable {
    ...
    AnimatedForgeTexture(Minecraft mc,DecodedImage decoded){this.mc=mc;this.decoded=decoded;texture=new DynamicTexture(decoded.width(),decoded.height(),true);upload(decoded.frames().get(0).image());location=mc.getTextureManager().getDynamicTextureLocation("picturebridge",texture);next=System.nanoTime()+decoded.frames().get(0).durationMillis()*1_000_000L;}
    ...
    private void upload(BufferedImage image){NativeImage pixels=texture.getTextureData();for(int y=0;y<image.getHeight();y++)for(int x=0;x<image.getWidth();x++)pixels.setPixelRGBA(x,y,abgr(image.getRGB(x,y)));texture.updateDynamicTexture();}
    private static int abgr(int c){return c&0xFF00FF00|c>>16&0xFF|(c&0xFF)<<16;}@Override public void close(){mc.getTextureManager().deleteTexture(location);}
}
