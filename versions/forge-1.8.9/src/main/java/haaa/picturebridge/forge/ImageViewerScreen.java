...
final class ImageViewerScreen extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        else drawCenteredString(fontRendererObj, tr("picturebridge.status.loading", dots()),
                    area.centerX(), area.centerY() - 4, 0xD9E2F2);

        drawCenteredString(fontRendererObj, tr("picturebridge.screen.title"), width / 2, 8, 0xFFFFFF);
        String status = System.nanoTime() < copiedUntilNanos
                ? tr("picturebridge.status.copied") : tr("picturebridge.status.hint");
        drawCenteredString(fontRendererObj, status, width / 2, Math.max(0, height - 45),
    ...
    private void drawError(ImageArea area) {
        ...
        drawCenteredString(fontRendererObj, tr("picturebridge.error.title"), area.centerX(), y, 0xFF6B6B);
        List<String> lines = fontRendererObj.listFormattedStringToWidth(errorText, contentWidth);
        for (String line : lines) {
            drawCenteredString(fontRendererObj, line, area.centerX(), y += 11, 0xD5D9E2);
        }
    }
